...
public abstract class Functional {
    ...
    public static <T> Option<T> find(T[] elements, Function1<? super T, Boolean> filter) {
        return find(Arrays.asList(elements), filter);
    }

    public static <T> Option<T> find(Iterable<T> elements, Function1<? super T, Boolean> filter) {
        return headOption(filter(elements, filter));
    }

    public static <T> Iterable<T> filter(T[] elements, Function1<? super T, Boolean> filter) {
        return filter(Arrays.asList(elements), filter);
    }

    public static <T> Iterable<T> filter(Iterable<T> elements, Function1<? super T, Boolean> filter) {
    	return new FilteringIterable<T>(elements, filter);
    }

    public static <T, E> Map<T, E> filter(Map<T, E> map, Function1<Map.Entry<? super T, ? super E>, Boolean> filter) {
        return Collections.newMap(filter(map.entrySet(), filter));
    }

    public static <S, T> Iterable<T> map(S[] elements, Function1<? super S, ? extends T> transformer) {
        return map(Arrays.asList(elements), transformer);
    }

    public static <S, T> Iterable<T> map(Iterable<S> elements, Function1<? super S, ? extends T> transformer) {
    	return new TransformingIterable<S,T>(elements, transformer);
    }

    public static <K1, V1, K2, V2> Map<K2, V2> map(Map<K1, V1> source, Function1<Map.Entry<K1, V1>, Map.Entry<K2, V2>> transformer) {
        return Collections.newMap(map(source.entrySet(), transformer));
    }

    public static <S, T> Iterable<T> flatMap(S[] elements, Function1<? super S, ? extends Iterable<T>> transformer) {
        return flatMap(Arrays.asList(elements), transformer);
    }

    public static <S, T> Iterable<T> flatMap(Iterable<S> elements, Function1<? super S, ? extends Iterable<T>> transformer) {
    ...
    public static <T> Iterable<T> flatten(Iterable<? extends Iterable<T>> elements) {
    ...
    public static <T,E> boolean contains(Iterable<T> elements, T element) {
        return exists(elements, Predicates.equal(element));
    }

    public static <T> boolean exists(T[] elements, Function1<T, Boolean> filter) {
        return exists(Arrays.asList(elements), filter);
    }

    public static <T> boolean exists(Iterable<T> elements, Function1<? super T, Boolean> filter) {
        return !isEmpty(filter(elements, filter));
    }

    public static <T> boolean forAll(T[] elements, Function1<? super T, Boolean> filter) {
        return forAll(Arrays.asList(elements), filter);
    }

    public static <T> boolean forAll(Iterable<T> elements, Function1<? super T, Boolean> filter) {
    ...
    public static <T> Iterable<T> concat(final Iterable<? extends T> elements1, final Iterable<? extends T> elements2, final Iterable<? extends T> elements3, final Iterable<? extends T> elements4) {
        return concat(elements1, concat(elements2, concat(elements3, elements4)));
    }
    ...
    public static <T> T reduce(Iterable<T> elements, Monoid<T> m) {
     ...
     */
    public static <T> Option<T> fold(T[] elements, Function2<T, T, T> f) {
     ...
     * @return <i>None</i> if <i>elements</i> is empty
     */
    public static <T> Option<T> fold(Iterable<T> elements, Function2<T, T, T> f) {
        if (isEmpty(elements)) {
            return None();
        }
        T ret = head(elements);
    ...
    public static <T extends Comparable<T>> T min(T e1, T... elements) {
        return min(concat(Arrays.asList(e1), Arrays.asList(elements))).get();
    }
    ...
    public static <T extends Comparable<T>> T max(T e1, T... elements) {
        return max(concat(Arrays.asList(e1), Arrays.asList(elements))).get();
    }
